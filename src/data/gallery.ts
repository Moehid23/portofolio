export interface GalleryItem {
  id: string;
  src: string;
  alt: string;
  category: string;
  description?: string;
  date?: string;
  location?: string;
  span?: string; // for masonry layout (e.g., "col-span-2 row-span-2")
}

export const galleryItems: GalleryItem[] = [
  {
    id: "1",
    src: "/wisuda.webp",
    alt: "Graduation Ceremony (Wisuda Sarjana Komputer)",
    category: "Graduation",
    description: "Graduation ceremony celebrating the completion of my Bachelor of Computer Science (S.Kom.) in Informatics Engineering at Universitas Buana Perjuangan Karawang with a GPA of 3.80.",
    date: "2026",
    location: "Universitas Buana Perjuangan Karawang",
    span: "md:col-span-2 md:row-span-2"
  },
  {
    id: "2",
    src: "/sidang_ta.webp",
    alt: "Bachelor Thesis Defense & Seminar",
    category: "Academic",
    description: "Successful defense of my undergraduate thesis on applying Random Forest machine learning to predict spare part inventory demand.",
    date: "2026",
    location: "Faculty of Computer Science, UBP Karawang",
    span: "md:col-span-2 md:row-span-1"
  },
  {
    id: "3",
    src: "/iaka_competition_2025.webp",
    alt: "IAKA Competition 2025",
    category: "Competition",
    description: "Participating in the IAKA Competition 2025, solving practical engineering and technical challenges.",
    date: "2025",
    location: "IAKA Arena",
    span: "md:col-span-2 md:row-span-1"
  },
  {
    id: "4",
    src: "/seminar_kkn.webp",
    alt: "Community Service Seminar (KKN)",
    category: "Community",
    description: "Presenting technological community development programs and digital literacy initiatives during the university community service (KKN) program.",
    date: "2025",
    location: "Karawang Regency",
    span: "md:col-span-2 md:row-span-1"
  },
  {
    id: "5",
    src: "/clash_of_akhlak.webp",
    alt: "Clash of Akhlak Event",
    category: "Event",
    description: "Participating in team building, leadership workshops, and collaborative sessions at the Clash of Akhlak event.",
    date: "2024",
    location: "Peruri Corporate Center",
    span: "md:col-span-2 md:row-span-1"
  },
];
